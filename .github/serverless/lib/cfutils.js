...
class WranglerCmd {
  ...
  publishProject() {
    ...
    const wranglerCmd = `wrangler pages publish _build/discuss --project-name ${projectName} --branch ${branch}`;
  ...
  createAppDbTables() {
    ...
    const wranglerCmd = `wrangler d1 execute ${dbName} --file _build/discuss/db.sql --remote`;
    console.log(wranglerCmd);
    return this._getCmd(wranglerCmd);
  }
