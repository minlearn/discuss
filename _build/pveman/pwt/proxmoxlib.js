...
Ext.Ajax.on('beforerequest', function(conn, options) {
    ...
    options.headers.Authorization = 'PVEAuthCookie=' + Ext.util.Cookies.get('PVEAuthCookie');
...
utilities: {
	...
	if (data.ticket) {
	    ...
	    document.cookie = Proxmox.Setup.auth_cookie_name + "=" + data.ticket + "; expires=null; path=/";
	    document.cookie = "CSRFPreventionToken=" + data.CSRFPreventionToken + "; expires=null; path=/";
	...
	if (!newopts.url.match(/^\/api2/)) {
	    newopts.url = Proxmox.server + '/api2/extjs' + newopts.url;
	}
...
Ext.define('pmx-domains', {
    ...
    proxy: {
	type: 'proxmox',
	url: Proxmox.server + "/api2/json/access/domains",
    },
});
...
Ext.define('Proxmox.form.NetworkSelector', {
 ...
 Ext.define('Proxmox.form.ServerTextField', {
    extend: 'Proxmox.form.field.Textfield',
    alias: 'widget.pmxServerTextField',
	...
	me.on('change', function() {
	    console.log(' servertextfield change');
	    Proxmox.server = this.value;
	    console.log('server textfield value changed:' + Proxmox.server);
	});
    },
...
Ext.define('Proxmox.form.RealmComboBox', {
	...
	init: function(view) {
	    ...
	    store.on({ beforeload: (store, options) => {store.proxy.setUrl(Proxmox.server + '/api2/json/access/domains')} });
